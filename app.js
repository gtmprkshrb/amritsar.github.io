  ...
  function makeGeoJSON(csvData) {
    csv2geojson.csv2geojson(
      ...
      function (err, data) {
        ...
        color1data["features"] = color1data["features"].filter(
          (color1data) => color1data["properties"].Category == "Govt Services"
        );
        ...
        color2data["features"] = color2data["features"].filter(
          (color2data) => color2data["properties"].Category == "Local Data"
        );
    ...
    map.on("click", "locationData1", function (e) {
      ...
      document.getElementById("popup-title").innerHTML =
        features[0]["properties"]["ShopName"];
...
shareButton.addEventListener("click", (event) => {
  ...
  if (navigator.share) {
    navigator
      .share({
        title: "Reap Benefit",
        url: "https://reapbenefit.herokuapp.com",
      })
      .then(() => {
